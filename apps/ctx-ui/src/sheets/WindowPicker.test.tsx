...
describe("Ctx window picker", () => {
  ...
  it("lists live windows and disables windows already in the workspace", async () => {
    render(<WindowPicker workspace="coding" onClose={vi.fn()} onAdded={vi.fn()} />);

    expect(await screen.findByText("Ctx editor")).toBeInTheDocument();
    expect(screen.getByText("Already in this context")).toBeInTheDocument();
    const checkboxes = screen.getAllByRole("checkbox");
    expect(checkboxes[0]).toBeEnabled();
    expect(checkboxes[1]).toBeDisabled();
  });

  it("adds the selected windows and reports completion", async () => {
    const onAdded = vi.fn();
    render(<WindowPicker workspace="coding" onClose={vi.fn()} onAdded={onAdded} />);
